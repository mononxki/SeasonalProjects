...
function enterAndRenameFolder(folderName) {
    ...
    fs.readdir(folderPath, (err, files) => {
        ...
        let maxDigits = Math.floor(Math.log10(files.length)) + 1;

        files.forEach((file, index) => {
            const extension = path.extname(file);
            const newFileName = `${(index + 1).toString().padStart(maxDigits, '0')}${extension}`;
        ...
        console.log(`Files in "${folderName}" have been renamed.`);
    });
}
